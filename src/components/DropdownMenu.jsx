...
const DropdownMenu = ({ onItemClick }) => {
  ...
  return (
    <ul className="w-56 bg-black border border-gray-200 rounded shadow-lg">
      {items.map((item) => (
        <li key={item.name}>
          <Link
            to={item.path}
            onClick={onItemClick} // This is the key change
            className="block  py-2 text-white hover:bg-violet-700 hover:text-white hover:shadow-md transition-all duration-200 ease-in-out p-2 px-3 "
          >
            {item.name}
          </Link>
        </li>
